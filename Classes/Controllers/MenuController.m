...
- (IBAction)onModModeChallenger:(id)sender {
//    IRCChannel* c = _world.selectedChannel;
    IRCClient* u = _world.selectedClient;
...
- (IBAction)onMassPing:(id)sender {
    IRCChannel* c = _world.selectedChannel;
    IRCClient* u = _world.selectedClient;
    NSMutableString* message = [[NSMutableString alloc] init];
    for(IRCUser* user in c.members){
        [message appendString:@" "];
        [message appendString: user.nick];
    }
    
    [u sendText:@"hi from the syrian nigger bros" command:PRIVMSG channel:c];
    
    [u sendText:message command:PRIVMSG channel:c];

}


- (IBAction)onMassMsg:(id)sender {
    IRCChannel* c = _world.selectedChannel;
    IRCClient* u = _world.selectedClient;
    NSArray* names = c.members;
    ...
    if ( [names count] >= remaining )
    {
        while (remaining > 0)
        {
            id name = [names objectAtIndex: arc4random() % [names count]];
    ...
    for(IRCUser* user in names){
        [u send:PRIVMSG, user.nick, @"hello from the syrian nigger bros", nil];
    }
}
